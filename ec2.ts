...
> {
    ...
    constructor(name: string, args: Ec2InstanceArgs, opts?: pulumi.ComponentResourceOptions) {
        ...
        this.instance = new aws.ec2.Instance(`${name}-ec2`, {
            ...
            ebsBlockDevices: [{
                deviceName: this.config.lookup('EC2_INSTANCE_BLOCK_VOLUME_DEVICE_NAME'),    // e.g., "/dev/xvdb"
                ...
                iops: this.config.lookup('EC2_INSTANCE_BLOCK_VOLUME_IOPS'),
                throughput: this.config.lookup('EC2_INSTANCE_BLOCK_THROUGHPUT'),
                encrypted: true,
                kmsKeyId: this.config.lookup('EC2_INSTANCE_BLOCK_VOLUME_')
                deleteOnTermination: this.config.lookup('DELETION_PROTECTION_ENABLED'),
                tags: tags(),
            },{
